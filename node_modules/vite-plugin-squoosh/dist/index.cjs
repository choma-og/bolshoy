...
const getFileId = (path2) => new Uint8Array(fs.readFileSync(path2).buffer.slice(-8)).reduce((id, byte) => id + byte, "");
...
function squooshPlugin(options = {}) {
  ...
  return {
    ...
    configResolved(resolvedConfig) {
      ...
      logger = options.silent ? vite.createLogger("silent") : config.logger;
    ...
    async closeBundle() {
      ...
      const imagePool = new lib.ImagePool(cpus);
